...
$sql = "SELECT DISTINCT u.id, u.name 
        ...
        JOIN chat_messages cm ON 
            (cm.sender_id = u.id AND cm.receiver_id = '$owner_id') OR 
            (cm.sender_id = '$owner_id' AND cm.receiver_id = u.id)
        WHERE u.role = 'user'
        ORDER BY u.name";
    ...
    <div class="chat-list">
        <?php if (mysqli_num_rows($result) > 0): ?>
            <?php while ($user = mysqli_fetch_assoc($result)): 
                ...
                $last_msg_q = mysqli_query($conn, "SELECT message, created_at FROM chat_messages 
                    WHERE (sender_id = '{$user['id']}' AND receiver_id = '$owner_id') 
                    OR (sender_id = '$owner_id' AND receiver_id = '{$user['id']}')
                    ORDER BY id DESC LIMIT 1");
                $last_msg = mysqli_fetch_assoc($last_msg_q);
                $last_time = !empty($last_msg['created_at']) ? date("h:i A", strtotime($last_msg['created_at'])) : "";
            ?>
                <a href="../chat/chat.php?user_id=<?= $user['id'] ?>" class="chat-item">
                    <div class="chat-avatar">
                        <?= strtoupper(substr($user['name'], 0, 1)) ?>
                    </div>
                    
                    <div class="chat-details">
                        <div class="chat-name"><?= htmlspecialchars($user['name']) ?></div>
        ...
        <?php else: ?>
            <div class="empty-state">
                ...
                <p>Your customers haven't contacted you yet</p>
                <a href="manage_bookings.php" style="
                    display: inline-block;
                    background: #27ae60;
                    color: white;
                    padding: 10px 25px;
